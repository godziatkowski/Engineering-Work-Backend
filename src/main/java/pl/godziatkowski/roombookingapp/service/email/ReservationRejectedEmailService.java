...
@Service
public class ReservationRejectedEmailService
    implements IReservationRejectedEmailService {

    private static final String EMAIL_TITLE = "Twoja rezerwacja została odrzucona";
    ...
    @Autowired
    public ReservationRejectedEmailService(JavaMailSender mailSender,
    ...
    @Override
    public void sendNotificationAboutRejectedReservation(UserSnapshot userSnapshot,
        ReservationSnapshot reservationSnapshot) throws EmailSendingException {

        LOGGER.info("Sending notification about rejected reservation for room <{}>at <{}> between <{}> - <{}> to <{}>",
        ...
        try {
            ...
            LOGGER.error("Unable to send notice about rejected reservation because of MessagingException <{}>",
                me.getMessage(), me);
            throw new EmailSendingException();
        } catch (RuntimeException re) {
            LOGGER.error("Unable to send notice about rejected reservation because of RuntimeException <{}>",
                re.getMessage(), re);
            throw new EmailSendingException();
        }
    }
    ...
    private void sendEmail(UserSnapshot userSnapshot, ReservationSnapshot reservationSnapshot)
        ...
        final String htmlContent = this.templateEngine.process("rejectedReservation", context);
        // true = isHtml
        message.setText(htmlContent, true);

        this.mailSender.send(mimeMessage);

        LOGGER.info("Sent notice about rejected reservation to <{}>", userSnapshot.getLogin());
    }
